...
class SegmentTree {
public:
    SegmentTree(int n, Merge merge, Identity id) : SegmentTree(vector<T>(n, id()), merge, id) {
    }

    SegmentTree(const vector<T>& v, Merge merge, Identity id)
        : n((int)v.size()), data(4 * n), merge(std::move(merge)), id(std::move(id)) {
        build(0, 0, n - 1, v);
    }
...
private:
    ...
    T query(int node, int l, int r, int ql, int qr) const {
        ...
        int m = (l + r) / 2;
        return merge(query(2 * node + 1, l, m, ql, qr), query(2 * node + 2, m + 1, r, ql, qr));
    }
